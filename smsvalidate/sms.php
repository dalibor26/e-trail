...
$baza = mysql_query("INSERT INTO billing_sms SET sender='".$sender."', message='".$message."',`time` = '".time()."',revenue='".$revenue."',billing_type='".$billing_type."',message_id='".$message_id."',username='".$username."',keyword='".$keyword."',country='".$country."',price='".$price."',currency='".$currency."',shortcode='".$shortcode."',status='".$status."',operator='".$operator."'");
  if($currency == "MKD"){
	  ...
	  $reply = "Uspesno ste kupili ste bon Цена:$novacplus eur za $message nalog Подршка: morenja@hotmail.com by fortumo.com";
	  }
      echo($reply);	  
}else if($currency == "BAM"){
	  if (mysql_num_rows($user) == 0)
      {
	  echo "Takav username ne postoji u bazi. Cena: $price BAM + PDV Support: morenja@hotmail.com, http://morenja.info/";
	  die();
      } 
	  $novacplus = $price / 1.959;
	  $upis = mysql_query("UPDATE `klijenti` SET `novac`=`novac`+'{$novacplus}' WHERE `username`='{$username}'");  
	  $reply = "Dodata uplata u iznosu od Cena:$price BAM + PDV na $message nalog Podrska: morenja@hotmail.com by fortumo.com";
  echo($reply);
  }
  }
...
$baza = mysql_query("INSERT INTO billing_sms SET sender='".$sender."', message='".$message."',`time` = '".time()."',revenue='".$revenue."',billing_type='".$billing_type."',message_id='".$message_id."',username='".$username."',keyword='".$keyword."',country='".$country."',price='".$price."',currency='".$currency."',shortcode='".$shortcode."',status='".$status."',operator='".$operator."'");
 if($currency == "RSD"){
	  ...
	  if (mysql_num_rows($user) == 0)
      {
	  echo "Takav username ne postoji u bazi. Cena: $price RSD + 1 Standard SMS Support: morenja@hotmail.com, http://morenja.info/";
	  die();
      }
	  $upis = mysql_query("UPDATE `klijenti` SET `novac`=`novac`+'{$novacplus}' WHERE `username`='{$username}'");  
	  $reply = "Upravo ste kupili Morenja Hosting bon za $price RSD + 1 Standard SMS Hvala! Podrska: morenja@hotmail.com by fortumo.rs";
  echo($reply);
	  ...
	  if (mysql_num_rows($user) == 0)
      {
	  echo "Takav username ne postoji u bazi. Cijena: $price HRK Support: morenja@hotmail.com, http://morenja.info/";
	  die();
      }
	  $upis = mysql_query("UPDATE `klijenti` SET `novac`=`novac`+'{$novacplus}' WHERE `username`='{$username}'");  
	  $reply = "Kupili ste Morenja Hosting bon za $price HRK Hvala vam! Podrska za placanje: morenja@hotmail.com by fortumo.rs";
  echo($reply);
  } else if($currency == "BAM"){
	  if (mysql_num_rows($user) == 0)
      {

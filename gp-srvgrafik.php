...
$sql = "SELECT igraci, igraci_5min, cache FROM `lgsl` WHERE ip = '$ipadresa' AND q_port = '$port' AND `id` = '$serverid'";

if(query_numrows($sql) == 0) die("Taj server ne postoji u bazi!".$ipadresa.'-'.$port.'-'.$serverid);
...
$cache_path2 = "grafik/$ipadresa:$port.png";
$cache_path1 = "grafik/blue/$ipadresa:$port.png";

if($_SESSION['style'] == 'blue') $cache_path = "grafik/blue/$ipadresa:$port.png"; 
else $cache_path = "grafik/$ipadresa:$port.png";
$cache_path2 = str_replace(":", "_", $cache_path2);
$cache_path1 = str_replace(":", "_", $cache_path1);
...
else 
{
	...
	$podaci = unserialize($server2['cache']);
	$tete =  $server2['igraci'];
	$playersmax = $podaci['s']['playersmax'];

	$temp=explode(":",$server2["igraci"]);
	...
	$bg_color = imagecolorallocate($image,240,240,255);
	$border = imagecolorallocate($image,255,255,255);
	$bjela = imagecolorallocate($image,255, 255, 255);
	$zuta = imagecolorallocate($image,255, 170, 0);

	$et2 = imagecolorallocate($image,52, 13, 23);
	$et1 = imagecolorallocate($image,10, 27, 36);

	if($_SESSION['style'] == 'blue') $et = imagecolorallocate($image,10, 27, 36);
	else $et = imagecolorallocate($image,52, 13, 23);
	...
	for($i=0;$i<=$hlin;$i++)
	{
		...
		imageline($image, $margins-1, $y, $tot_width - $margins+6, $y, $grey);	
	...
	imagefill($image, 0, 0, $et1);
	imagepng($image);
	imagepng($image, $cache_path1);

	imagefill($image, 0, 0, $et2);
	imagepng($image);
	imagepng($image, $cache_path2);
